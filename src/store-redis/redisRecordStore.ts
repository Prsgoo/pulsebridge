...
export class RedisRecordStore implements RecordStore {
  ...
  // Serializes concurrent setByPlugin calls for the same pluginId, preventing
  // races between the smembers read and the subsequent MULTI/EXEC write.
  ...
  async setByPlugin(
    ...
    // Serialize writes per plugin so the smembers read and MULTI/EXEC are never
    // interleaved with another setByPlugin call for the same pluginId.
    ...
    // Store a settled version so the queue entry doesn't block future calls on error.
  ...
  private async doSetByPlugin(
    ...
    await withFallback(
      this.logger,
      async () => {
        // Read the types this plugin previously produced so we can clean up stale index entries.
        ...
        // Remove type index entries for types this plugin no longer produces.
        ...
        // Replace type index entries for types this plugin now produces.
