...
import type { WebhookDefinition } from "../webhooks/webhookDefinition.js";
...
export interface IntegrationPluginManifest {
  ...
  /** Outbound actions the plugin can perform on demand (requires `invoke()`). */
  actions?: ReadonlyArray<ActionDefinition>;
  /** Declares the plugin accepts inbound webhook events (requires `ingest()`). */
