...
describe("InMemorySecretStore", () => {
  ...
  describe("empty key", () => {
    it("get returns undefined even when an empty key holds a value", () => {
      const store = new InMemorySecretStore({ "": "non-empty" });
      expect(store.get("")).toBeUndefined();
    });

    it("has returns false even when an empty key holds a value", () => {
      const store = new InMemorySecretStore({ "": "non-empty" });
  ...
  describe("empty value", () => {
    it("get treats an empty-string value as absent", () => {
      const store = new InMemorySecretStore({ EMPTY: "" });
      expect(store.get("EMPTY")).toBeUndefined();
    });

    it("has treats an empty-string value as absent", () => {
      const store = new InMemorySecretStore({ EMPTY: "" });
