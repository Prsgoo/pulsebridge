...
export interface RuntimeContext {
  ...
  /**
   * Aborted when the platform's per-call execution timeout fires. Long-running
   * plugins should forward it to cancellable work (e.g. `fetch(url, { signal })`)
   * so a timed-out call releases its sockets instead of running to completion in
   * the background. Present during `execute()`/`process()`/`reauth()` calls.
   */
  signal?: AbortSignal;
}
