...
describe("Platform integration — EnvSecretStore", () => {
  it("reads integration secrets from process.env", async () => {
    vi.useFakeTimers();
    const { EnvSecretStore } =
      await import("../../contracts/secrets/envSecretStore.js");
    process.env["TEST_API_KEY"] = "test-value-123";

    try {
      const core = new PulseBridgeCore({ secrets: new EnvSecretStore() });
      let capturedSecrets: string | undefined;

      await core.registerIntegration({
        manifest: {
          id: "test-env-integration",
          name: "Env Test Integration",
          version: "1.0.0",
          kind: "integration",
          operations: [
            {
          ...
          auth: {
            type: "apiKey",
            secrets: [{ key: "TEST_API_KEY", required: true }],
          },
          polling: { defaultIntervalMs: 60_000, hard: true },
        },
        async execute(
          ...
          capturedSecrets = context.secrets.get("TEST_API_KEY");
          return [];
        },
      });
      ...
      await core.start();
      await core.waitForReady();
      expect(capturedSecrets).toBe("test-value-123");
      await core.stop();
    } finally {
...
describe("Platform integration — OAuth2 TokenStore", () => {
  it("proactively sets needs_reauth when a stored token is expired", async () => {
    ...
    // Token was expired → reauth triggered → no reauth() impl → skipped, status remains needs_reauth
...
describe("Platform integration — processor chaining (consumesViews)", () => {
  it("passes upstream view data to a chained processor", async () => {
    ...
    // Chained processor that consumes the sensor summary view
    await core.registerProcessor({
      manifest: {
